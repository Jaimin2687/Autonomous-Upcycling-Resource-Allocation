...
inputs = {
  ...
  node_type               = try(local.redis_conf.node_type, "cache.t4g.small")
  engine_version          = try(local.redis_conf.engine_version, "7.1")
  auth_token              = get_env(local.redis_conf.auth_token_env_var, "")
  multi_az                = try(local.redis_conf.multi_az, false)
  replica_count           = try(local.redis_conf.replica_count, 0)
  snapshot_retention_days = try(local.redis_conf.snapshot_retention, 1)
  ...
  apply_immediately       = try(local.redis_conf.apply_immediately, true)
  tags                    = local.tags
}
