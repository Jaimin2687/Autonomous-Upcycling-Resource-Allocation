...
inputs = {
  ...
  instance_class          = try(local.rds_conf.instance_class, "db.t4g.medium")
  allocated_storage       = try(local.rds_conf.allocated_storage, 20)
  max_allocated_storage   = try(local.rds_conf.max_allocated_storage, 100)
  ...
  multi_az                = try(local.rds_conf.multi_az, false)
  backup_retention_days   = try(local.rds_conf.backup_retention_days, 1)
  deletion_protection     = try(local.rds_conf.deletion_protection, false)
  final_snapshot_on_delete = try(local.rds_conf.final_snapshot_on_delete, false)
  ...
  enhanced_monitoring_interval = try(local.rds_conf.enhanced_monitoring_int, 0)
  monitoring_role_arn     = try(local.rds_conf.monitoring_role_arn, null)
  apply_immediately       = try(local.rds_conf.apply_immediately, true)
  tags                    = local.tags
}
