...
inputs = {
  ...
  instance_class          = try(local.rds_conf.instance_class, "db.m6g.xlarge")
  allocated_storage       = try(local.rds_conf.allocated_storage, 200)
  max_allocated_storage   = try(local.rds_conf.max_allocated_storage, 500)
  ...
  backup_retention_days   = try(local.rds_conf.backup_retention_days, 7)
