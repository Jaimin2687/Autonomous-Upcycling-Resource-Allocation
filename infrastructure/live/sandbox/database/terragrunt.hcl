...
inputs = {
  ...
  instance_class          = try(local.rds_conf.instance_class, "db.m6g.large")
  allocated_storage       = try(local.rds_conf.allocated_storage, 100)
  max_allocated_storage   = try(local.rds_conf.max_allocated_storage, 300)
  ...
  backup_retention_days   = try(local.rds_conf.backup_retention_days, 3)
